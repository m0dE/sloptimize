 ...
 * with no phase is never guessed into one: it answers only to `?`, the name
 * the catalogue gives it (as buildIssues' `phase` does).
 */
export function onlyPhases(records, phases) {
  if (!phases) return records;
  return records.filter((r) => phases.has(typeof r?.phase === 'string' && r.phase ? r.phase : '?'));
}

...
function strip(label, unit, values, kind, marks, cw = W, ranges = []) {
  ...
  if (kind === 'line') {
    ...
    // A build measured in several runs: the runs' range as a whisker, so a
    // bar that moved less than its own spread reads as noise, not a fix.
    ranges.forEach((r, i) => {
      if (!r || typeof r.lo !== 'number' || typeof r.hi !== 'number' || r.hi <= r.lo) return;
      const cx = (x(i) + xw / 2).toFixed(1);
      drawn += `<line class="sl-spread" x1="${cx}" x2="${cx}" y1="${y(r.hi).toFixed(1)}" y2="${y(r.lo).toFixed(1)}" stroke="${C.ink}" stroke-opacity="0.7" stroke-width="1"/>`;
    });
  }
...
export function stripsSvg(h) {
  ...
  const columns = [
    ...
    ['hitches/h', '', b.map((k) => k.hitchesPerHour), 'bars', b.map((k) => k.spread)],
...
export function createPanel(host) {
  ...
  function wireCrosshair(h) {
    ...
    svg.addEventListener('mousemove', (e) => {
      ...
      read.textContent = `build ${k.build}  ${when(k.from)} → ${when(k.to)}  p95 ${num(k.p95Ms, 'ms')}${typeof k.bodyMs === 'number' ? `  body ${num(k.bodyMs, 'ms')}` : ''}  calls ${num(k.calls)}  hitches/h ${num(k.hitchesPerHour)}${k.spread ? ` (${k.spread.n} runs ${num(k.spread.lo)}–${num(k.spread.hi)})` : ''}${k.worstMs ? ` (worst ${num(k.worstMs, 'ms')} ${k.worstGuess ?? ''})` : ''}${secs ? `  │ ${secs}` : ''}`;
    });
